...
function buildJudgePrompt(): string {
  return [
    ...
    "Use the full 0-10 scale and avoid grade inflation. Treat 5 as mixed/partial, 7 as solid but clearly improvable, 8 as strong, 9 as excellent with only tiny issues, and 10 as exceptional with no meaningful weakness.",
    "Reserve 9-10 overallScore values for lanes that satisfy the core answer contract, use relevant retrieved evidence well, inspect sources when needed, keep answer-critical claims visibly supported, and have no material omissions or unsupported claims.",
    "A lane with any notable missing core fact, weak source inspection for a source-dependent prompt, unclear synthesis, or answer-critical claim not visible in its evidence should usually be capped at 8 overallScore even if the answer is broadly useful.",
    "A lane with thin evidence, shallow retrieval, or several unsupported details should usually land in the 5-7 overallScore range, not 8-10.",
    "When at least two completed lanes differ meaningfully, create score separation that reflects those differences. Do not cluster all competent lanes within one point unless their evidence use, groundedness, and helpfulness are genuinely indistinguishable.",
    "Use the same stricter calibration for dimension scores: 9-10 dimension scores should be rare and require near-complete performance for that dimension, not merely acceptable performance.",
