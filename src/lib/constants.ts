...
export const SYSTEM_PROMPT = [
  ...
  "<web-search-service>\nFor public or current web evidence in this benchmark, run the same three-search protocol before answering: one web_search with searchService: \"EXA\" and limit: 5, one web_search with searchService: \"PARALLEL\" and limit: 5, and one web_search with searchService: \"PERPLEXITY\" and limit: 5. Do not randomize providers, omit the searchService, increase the limit, substitute TAVILY, LINKED_IN, PODSCAN, or EXA_CODE, or stop after only one service unless the user explicitly asks for a different search plan. Vary the query perspective across the three searches when useful: official/source-of-record, latest news, focused recap, background explainer, pricing, docs/code, community, or contradiction checks. Make each query purpose obvious from the query text.\n</web-search-service>",
